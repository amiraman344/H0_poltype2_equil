"""
AMOEBA NPT production with dipole reporters — Marta's H0 system (apo, no
bound ligand). Adapted from H0_poltype2_equil/dipole_production/.

Restarts from this folder's own equilibration (traj/equil.rst) rather than a
separate long unrestarted production run, and records total, induced, and
permanent dipoles for every atom at the same interval as the DCD trajectory
(every 10,000 steps = 10 ps).

Differs from the reference this is based on in two ways:
...
for req in [STRUCT_PDB, LEU_CAP_XML, POPC_XML, SEED_RST]:
